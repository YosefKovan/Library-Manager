...
class Library:
    ...
    # ========================================
    def perform_borrow(self, user, book):

        try:
            result = self.validate_book_and_user(user.user_id, book.isbn)
            if not result[0]:
                raise f"Error : {result[1]}"

            if not self.books[book.isbn].is_available:
                raise "the book is currently not available!!!"

            book.is_available = False
            user.borrow_book(book)
            Logger.action_type("INFO", "Book borrowed")
    ...
    def perform_return(self, user_id, isbn):
        try:
            ...
            if self.books[isbn].is_available:
               raise "The book is currently available. cannot return book!"
            ...
            Logger.action_type("INFO", "Book returned")
            SystemAdmin.update_transactions_count(1)
